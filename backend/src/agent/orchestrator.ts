...
import { WeeklyPlanSchema, TriageResultSchema } from '@focus/shared';
import { z } from 'zod';
...
export const triageInbox = async (userId: string, limit: number = 10) => {
    ...
    // 3. Call LLM
    const result = await callLLM({ userPrompt: prompt });
